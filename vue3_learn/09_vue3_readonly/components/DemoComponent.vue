<template>
    <h1>Name :{{ name }}</h1>
    <h1>toRef Name :{{ name }}</h1>
    <h1>Age :{{ age }}</h1>
    <h1>Job type :{{ job.type }}, salary: {{ job.salary }}</h1>
...
<script >
import { reactive, readonly, shallowReadonly, toRef, toRefs } from 'vue'
export default {
    name: 'DemoComponent',
    setup() {
        let person = reactive({
        ...
        let shallowReadOnlyPerson = shallowReadonly(person)
        return {
            shallowReadOnlyPerson,
            readOnlyPerson,
